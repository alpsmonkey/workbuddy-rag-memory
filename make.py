...
用法:
  ...
  python make.py test
  python make.py server
  python make.py server-rerank
  python make.py help
...
def cmd_install_dev():
    ...
    if rc != 0:
        return rc
    return run([sys.executable, "-m", "pip", "install", "-e", ".[test,watchdog,server]"])


def cmd_install_server():
    return run([sys.executable, "-m", "pip", "install", "-e", ".[server]"])


def cmd_server():
    """make.py server"""
    return run([venv_python(), "scripts/server.py", "--host", "127.0.0.1", "--port", "8000"],
               cwd=str(PROJECT_ROOT))


def cmd_server_rerank():
    """make.py server-rerank - 带 rerank + hyde"""
    return run([venv_python(), "scripts/server.py", "--host", "127.0.0.1", "--port", "8000",
                "--rerank", "--hyde"],
               cwd=str(PROJECT_ROOT))
...
COMMANDS = {
    ...
    "clean": cmd_clean,
    "server": cmd_server,
    "server-rerank": cmd_server_rerank,
}
