...
def _post_install(skill_name: str, target_dir: Path, verbose: bool = False) -> None:
    """安装后回调：为需要项目路径的 skill 写 config.json（绝对路径，~ 已展开）"""
    if skill_name != "rag_search":
        return  # 其他 skill 暂不需要
    ...
    config = {
        ...
        "note": "Auto-generated by install_skills.py. Re-run if PROJECT_ROOT or index_dir changes.",
    }
    cfg_path = target_dir / "config.json"
    cfg_path.write_text(json.dumps(config, ensure_ascii=False, indent=2), encoding="utf-8")
    _log(f"config.json → {cfg_path}", verbose, "ok")
...
def install_one(src_dir: Path, target_dir: Path, dry_run: bool = False, verbose: bool = False) -> dict:
    ...
    # 安装后回调：写 config.json（让 skill 开箱即用）
    _post_install(src_dir.name, target_dir, verbose=verbose)
